...
const LatestReviews = () => {
  ...
  const { data, error } = useSWR('/api/dashboard', getFetcher);
  ...
  const handleSearch = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (search === "") {
      // If search term is cleared, reset to original rows
      setRows(originalRows);
    } else {
      // Filter rows based on the search term
      const filteredRows: ProductType[] = originalRows.filter((row: any) =>
        row.pname.toLowerCase().includes(search.toLowerCase())
      );
      setSearch(event.target.value);
